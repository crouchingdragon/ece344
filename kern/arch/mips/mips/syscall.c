...
// 	struct thread* tp;
// 	tp = thread_getthepid(pid);
// 	// check if process has already exited (should already have a stored exitval)
// 	if (tp->exitcode != NULL){
// 		// just return
// 		*retval = (int)pid;
// 		status = tp->exitcode;
...
//     //Badflag, self/parent wait
//     if (options != 0 || pid == 0 || curthread->t_pid == pid || pid == curthread->parent) {
//         return EINVAL;
//     }

//     //invalid status pointer
//     int *kstatus = kmalloc(sizeof (int));
//     if (copyin((const_userptr_t) status, kstatus, sizeof (int))) {
//         kfree(kstatus);
//         return EFAULT;
//     }
//     kfree(kstatus);
...
//     return waitpid_helper(pid, status, retval);
// }

// NOT GOOD=======================
// int
// waitpid_helper(pid_t pid, int *status, int *retval) {
//     struct thread *th = thread_getthepid(pid);
...
//     if (thread_exited(th)) {
//         copyout(th->exitcode, (userptr_t) status, sizeof (int));
//         *retval = pid;
//         thread_destroyzombie(th);
//         return 0;
//     }
...
// exit code is being acquired through exit, no other way
int
sys_waitpid(pid_t pid, int* status, int options, int* retval){
